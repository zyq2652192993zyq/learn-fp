...
class TraversableOps[A, C[_]](xs: C[A])(implicit foldable: Foldable[C], traversable: Traversable[C]) {
  def traverse[B, F[_]](fx: A => F[B])(implicit functor: Functor[F], applicative: Applicative[F]): F[C[B]] =
    traversable.traverse(xs)(fx)
}

object TraversableOps {
  implicit def toTraversableOps[A, C[_]](xs: C[A])(implicit foldable: Foldable[C], traversable: Traversable[C]) =
    new TraversableOps[A, C](xs)
}

class SequenceOps[A, C[_], F[_]](xs: C[F[A]])(
    implicit foldable: Foldable[C],
...
object SequenceOps {
  implicit def toSequenceOps[A, C[_], F[_]](
      xs: C[F[A]]
  )(implicit functor: Functor[F], applicative: Applicative[F], foldable: Foldable[C], traversable: Traversable[C]) =
    new SequenceOps[A, C, F](xs)
}
